...
if __name__ == '__main__':
    ...
    with open("/home/pia/Schreibtisch/CLUWLL-Project/preprocessing_tatoeba/unique_lemmas_learner_texts.csv") as lemma_file:
        csv_reader = csv.reader(lemma_file, delimiter="\t")
        for row in csv_reader:
            unique_lemma = row[0]
            for id_sent, sent_lemmatized in selected_sents_ita_lemma.items():
                if unique_lemma in sent_lemmatized:
                    lemma_context[unique_lemma].append(int(id_sent))

    #dict_to_csv(lemma_context, "/home/pia/Schreibtisch/CLUWLL-Project/preprocessing_tatoeba/context_sentences.csv")
